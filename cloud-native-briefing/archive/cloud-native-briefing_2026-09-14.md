...
> 覆盖窗口：2026-09-10 至 2026-09-14（常规）

本期没有值得报告的内容：watchlist 六项（Kubernetes 本体、AWS EKS、Istio、Flux CD/Kustomize/Helm、Kyverno/SOPS、AWS Load Balancer Controller/NLB）逐一检索，窗口内除上期（2026-09-10）已报告的内容外无新发布、无新安全公告；方向 B（其他动态）与方向 C（趋势）四条门槛均未命中新事件；进行中事件表逐条定向核查（Istio GCP 制品断供测试尚未到 9-15 首次窗口、Kyverno apiCall/CEL 漏洞模式无新 CVE、SOPS Vault allowlist 默认值未随新版本落地、SOPS CNCF 治理状态无新结论、EKS 仍未支持 K8s 1.37），均无新进展。
